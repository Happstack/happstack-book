...
<dl>
 <dt>`&lt;&gt;`</dt>
 <dd>`&lt;&gt;` is the choice operator. It chooses between the various paths.</dd>
 <dt>`.`</dt>
 <dd>`.` is used to combine elements together.</dd>
 <dt>`&lt;/&gt;`</dt>
 <dd>the combinators, such as `lit`, `int`, `anyText`, operate on a single path segment. `&lt;/&gt;` matches on the / between path segments.</dd>
 <dt>`lit`</dt>
 <dd>`lit` matches on a string literal. If you enabled `OverloadedStrings` then you do not need to explicitly use the `lit` function. For example, you could just write, `int . "-" . anyText`.</dd>
 <dt>`int`</dt>
 <dd>`int` matches on an `Int`.</dd>
 <dt>`anyText`</dt>
 <dd>`anyText` matches on any string. It keeps going until it reaches the end of the current path segment.</dd>
 <dt>`xmaph`</dt>
 <dd>`xmaph` is a bit like `fmap`, except instead of only needing `a -> b` it also needs the other direction, `b -> Maybe a`.
...
~~~~
 ...
 <dt>longest route</dt>
 <dd>You will notice that the parser for `/users` comes before `/users/<i>int</i>-<i>string</i>`. Unlike `parsec`, the order of the parsers (usually) does not matter. We also do not have to use `try` to allow for backtracking. `boomerang` will find all valid parses and pick the best one. Here, that means the parser that consumed all the available input.</dd>
</dl>
